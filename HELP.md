...
Токены и другие секреты не должны храниться в исходном коде. Для локального запуска
скопируйте [.env.example](.env.example) в `.env` в корне проекта и заполните свои значения.
Если `.env` уже существует, сохраните его настройки.

Пример переменной для модуля `bot`:

```properties
TELEGRAM_TOKEN="<YOUR_TELEGRAM_BOT_TOKEN>"
```

В `application.yaml` свойство `app.telegram.token` читает переменную `TELEGRAM_TOKEN`.
Docker Compose читает `.env` для подстановки настроек инфраструктуры. Приложения,
запускаемые через `java -jar` или IDE, сами этот файл не загружают: передайте его значения
в окружение процесса, как описано в [README.md](README.md#переменные-окружения).

Файлы `.env` и `.env.*` исключены через `.gitignore`; `.env.example` содержит только
демонстрационные значения и может храниться в репозитории.

## Зависимости

Общие версии зависимостей и настройки сборки определены в корневом [pom.xml](./pom.xml).
Зависимости отдельных сервисов перечислены в `pom.xml` соответствующих модулей.
...
Интеграционные тесты используют [Testcontainers](https://java.testcontainers.org/)
для запуска зависимых сервисов в Docker. Актуальные образы и их версии заданы в коде
интеграционных тестов соответствующих модулей.
